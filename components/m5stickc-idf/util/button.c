...
esp_err_t button_ioinit(button_t *button, bool en_pullup);
...
esp_err_t button_ioinit(button_t *button, bool en_pullup)
{
    ...
    if(en_pullup){
        gpio_pullup_en(button->gpionumber);
    }
...
esp_err_t ButtonInit()
{
    ...
    e = button_ioinit(&button_a, false);
    if (e == ESP_OK)
    {
    ...
    e += button_ioinit(&button_b, false); // Notice += on error return to accumulate previous error
    if (e == ESP_OK)
    {
