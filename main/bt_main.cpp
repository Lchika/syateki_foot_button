...
static DigitalSwitch foot_switch(GPIO_NUM_32, 1, 50, false);
static bool is_wait_stop = false;
static int bullet_num = 0;
static int gun_num = 5;
...
void bt_main(void *arg)
{
    ...
    ifs.reset(new std::ifstream("/spiffs/shot1.wav", std::ios::binary));
    ifs->seekg(0, std::ios::end);
    ESP_LOGI(SPIFFS_TAG, "%s /spiffs/shot1.wav size: %lld", __func__, static_cast<long long int>(ifs->tellg()));
    
    ifs.reset(new std::ifstream("/spiffs/decision24.wav", std::ios::binary));
    ifs->seekg(0, std::ios::end);
    ESP_LOGI(SPIFFS_TAG, "%s /spiffs/desicion24.wav size: %lld", __func__, static_cast<long long int>(ifs->tellg()));
    
    ifs.reset(new std::ifstream("/spiffs/cancel3.wav", std::ios::binary));
    ifs->seekg(0, std::ios::end);
    ESP_LOGI(SPIFFS_TAG, "%s /spiffs/cancel3.wav size: %lld", __func__, static_cast<long long int>(ifs->tellg()));
    ifs->seekg(44);
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    if (eir) {
        ...
        //if (strcmp((char *)s_peer_bdname, "M5_SPEAKER_T1") != 0) {
        if (strcmp((char *)s_peer_bdname, "SoundCore 2") != 0) {
            return;
        }
...
static void bt_av_hdl_stack_evt(uint16_t event, void *p_param)
{
    ...
    case BT_APP_EVT_STACK_UP: {
        ...
        do {
            ...
            s_tmr = xTimerCreate("connTmr", (100 / portTICK_RATE_MS),
                               //pdTRUE, (void *)tmr_id, a2d_app_heart_beat);
                               pdTRUE, (void *)tmr_id, handle_external_action);
...
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    ...
    */
    ...
    for (int i = 0; i < len; i++){
        ifs->read(reinterpret_cast<char*>(data+i), sizeof(uint8_t));
        if(ifs->eof()){
            //data[i] = 0;
...
static void handle_external_action(void *arg)
{
    ...
    if(foot_switch.isOn()){
        ESP_LOGI(APP_MAIN_TAG, "state: %d", s_a2d_state);
        if(s_media_state == APP_AV_MEDIA_STATE_IDLE){
            ...
            TimerHandle_t timer_h = xTimerCreate("oneShotTmr", (2000 / portTICK_RATE_MS),
                                    pdFALSE, 0, stop_play);
            xTimerStart(timer_h, portMAX_DELAY);
            const size_t body_buff_len = 64;
            char body_buff[body_buff_len];
            http_get(body_buff, body_buff_len, gun_num);
            if(body_buff != NULL){
                target_num = atoi(body_buff);
            }
            ESP_LOGD(APP_MAIN_TAG, "response body: %s", body_buff);
            ESP_LOGI(APP_MAIN_TAG, "target_num: %d", target_num);
            is_wait_stop = true;
        }
    }
    if(is_wait_stop){
    ...
    case APP_AV_MEDIA_STATE_STOPPING: {
        if (event == ESP_A2D_MEDIA_CTRL_ACK_EVT) {
            ...
            if (a2d->media_ctrl_stat.cmd == ESP_A2D_MEDIA_CTRL_STOP &&
                ...
                ESP_LOGI(BT_AV_TAG, "a2dp media stopped successfully");
                s_media_state = APP_AV_MEDIA_STATE_IDLE;
...
void bt_set_bullet_num(int bullet_num_in){
    bullet_num = bullet_num_in;
}
...
void bt_set_gun_num(int gun_num_in){
    gun_num = gun_num_in;
}
